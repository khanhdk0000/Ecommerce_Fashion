...
def search(request):
    ...
    filter_gender = ~Q(product_gender=None) if gender is None else Q(product_gender__iexact=gender)
    filter_color = ~Q(product_color=None) if color is None else Q(product_color__icontains=color)
    filter_cat = ~Q(category=None)
    filter_subcat = ~Q(subcategory=None)

    if name:
        name_lst = name.split(' ')
        filter_name = reduce(lambda x, y: x & y, [Q(product_name__icontains=word) for word in name_lst])

    # Take the id of category and subcategory 
    if cat:
        cat_id = Category.objects.filter(name__iexact=cat).values_list('category_id', flat=True)[0]
        filter_cat = Q(category=cat_id)
    if subcat:
        subcat_id = SubCategory.objects.filter(name__iexact=subcat).values_list('subcategory_id', flat=True)[0]
        filter_subcat = Q(subcategory=subcat_id)

    try:
        products = Product.objects.filter(filter_name, filter_color, filter_gender, filter_cat, filter_subcat)[:20]
